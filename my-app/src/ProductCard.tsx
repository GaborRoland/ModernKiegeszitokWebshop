...
function ProductCard({ product, onAddToCart, onOpenDetails, isInCart, isAnimating, isFavorite, onToggleFavorite, isInCompare, onToggleCompare }: ProductCardProps) {
  ...
  const showDealBadge = product.price <= 10000;
  const showNewBadge = (product.rating ?? 0) >= 4.6;
  ...
  return (
    <div
      ...
      className={`group bg-amber-50/95 dark:bg-slate-800 p-5 md:p-6 rounded-2xl shadow-md hover:shadow-2xl transition-all duration-300 border border-amber-200 dark:border-slate-700 ${
        isAnimating ? 'scale-105 shadow-2xl' : ''
      } hover:-translate-y-1.5 hover:scale-[1.01] cursor-pointer`}
    >
      <div className="relative">
        {(showDealBadge || showNewBadge) && (
          <div className="absolute top-3 left-3 z-10 flex items-center gap-2">
            {showDealBadge && (
              <span className="px-2.5 py-1 rounded-full text-[11px] font-black tracking-wide bg-red-500 text-white shadow">
                AKCIÓ
              </span>
            )}
            {showNewBadge && (
              <span className="px-2.5 py-1 rounded-full text-[11px] font-black tracking-wide bg-emerald-500 text-white shadow">
                ÚJ
              </span>
            )}
          </div>
        ...
        <img
          ...
          className="w-full h-48 object-cover rounded-xl mb-4 shadow-md transition-transform duration-300 group-hover:scale-[1.02]"
          loading="lazy"
          onError={(e) => {
        ...
        <button
          ...
          className={`absolute top-3 right-14 w-10 h-10 rounded-full flex items-center justify-center text-xl transition-all duration-200 shadow-lg ${
            isInCompare
      ...
      <div className="space-y-2">
        ...
        <div className="flex items-end justify-between gap-3 flex-wrap">
          ...
          <button
            ...
            className={`px-4 py-2.5 rounded-xl font-bold tracking-wide transition-all duration-200 shadow-md hover:shadow-xl active:scale-95 ${
              isInCart
                ? 'bg-amber-500 text-amber-950 dark:bg-sky-600 dark:text-white hover:bg-amber-600 dark:hover:bg-sky-700'
                : 'bg-gradient-to-r from-amber-400 to-orange-400 text-amber-950 dark:from-sky-600 dark:to-cyan-600 dark:text-white hover:from-amber-500 hover:to-orange-500 dark:hover:from-sky-700 dark:hover:to-cyan-700'
            }`}
          >
            {isInCart ? 'Még egyet' : 'Kosárba'}
          </button>
        </div>
